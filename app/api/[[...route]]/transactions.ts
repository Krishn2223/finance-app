...
const app = new Hono()
  ...
  //adding middleware to confirm that only logged in user can access.
  clerkMiddleware(),
  async (c) => {
    ...
    const data = await db
      ...
      .where(
        and(
          ...
          //to ensure that only logged in users's transactions will be loaded
  ...
  .delete(
    ...
    async (c) =>{
     ...
     const  transactionsToDelete = db.$with("delete").as(
      db.select({ id: transactions.id }).from(transactions)
      .innerJoin(accounts, eq(transactions.accountId, accounts.id))
      .where(and(
        eq(transactions.id, id),
